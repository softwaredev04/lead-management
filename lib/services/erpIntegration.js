...
/**
 * Tell ERP to revoke the matching IntegrationConnection (CRM → ERP sync).
 * Tries canonical external-disconnect paths; never throws — returns result object.
 *
 * ERP must implement (Phase 5):
 *   POST /integrations/external-disconnect
 *   Header: X-Integration-Secret
 *   Body: { connectionId, companyId, targetSystem, externalConnectionId, reason }
 */
async function notifyErpDisconnect({
  connectionId,
  companyId,
  externalConnectionId,
  ...
  try {
    ...
    const paths = [
      "/integrations/external-disconnect",
      "/integrations/target/lead-crm/external-disconnect",
      connectionId
        ? `/integrations/${encodeURIComponent(connectionId)}/external-disconnect`
        : null,
    ].filter(Boolean);
    ...
    for (const path of paths) {
      try {
        ...
        // 404 = try next path; other errors stop with detail
        if (res.status === 404) {
          lastError = { status: 404, path, body };
          continue;
        }
        return {
          ok: false,
        ...
        lastError = { error: err.message, path };
      }
    }
    ...
    return {
      ...
      error:
        lastError?.error ||
        "ERP external-disconnect endpoint not found. ERP must add POST /integrations/external-disconnect",
      lastError,
    };
  } catch (err) {
